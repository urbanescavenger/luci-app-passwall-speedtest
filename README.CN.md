...
*   **限时迭代测速**：开启 `iterate_enabled` 后反复运行完整测速，直到累计时长达 `iterate_minutes` 分钟；第 2 轮起每个待测节点只测自己上一轮通过的 IP（各待测节点测速彼此独立），候选池逐轮收敛，留下最稳定的低延迟 IP。
...
*   **限时迭代测速 / 迭代测速时长**：开启后反复测到指定时长为止，第 2 轮起每个待测节点只测上轮各自通过的 IP。
